...
def parse_available_events(page):
    ...
    # Używamy elastycznego lokalizatora dla kart na liście
    ...
    for card in cards:
        try:
            ...
            if "wolne" in card_text_lower or "dostępne" in card_text_lower or "zarezerwuj" in card_text_lower:
                # Czyścimy tekst z podwójnych enterów i zbędnych spacji, żeby ładnie wyglądał w wiadomości
...
def check_reservio_detailed():
    ...
    with sync_playwright() as p:
        ...
        page = context.new_context().new_page() if context.pages else context.new_page()
