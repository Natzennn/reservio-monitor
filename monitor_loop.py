...
BASE_URL = "https://ttsd.reservio.com/events"  # zmień na swoją stronę, np: test1874.reservio.com
CHECK_EVERY_SECONDS = 180  # co ile sekund sprawdzać całość
SCAN_DAYS_AHEAD = 90       # 3 miesiące
PAGE_WAIT_MS = 5000
...
def parse_available_events(full_text, checked_day):
    ...
    for idx, line in enumerate(lines):
        if availability_regex.search(line):
            ...
            found_events.append(f"{checked_day.strftime('%Y-%m-%d')} | {' | '.join(context)}")
        ...
        try:
            ...
            if events:
                ...
                if current_details != last_found_details:
                    message = f"🚨 Reservio: wykryto dostępne miejsce!\n\n{current_details}\n\n{BASE_URL}"
                    notify(message)
            ...
            print("Zakończono pełne skanowanie 3 miesięcy.", flush=True)
